...
export const CartProvider = ({ children }) => {
  ...
  const fetchCart = async () => {
    try { setCartLoading(true); const res = await axios.get(`${API}/cart`); setCart(res.data.cart || []); }
    catch { setCart([]); } finally { setCartLoading(false); }
  };
  const addToCart = async (productId, quantity = 1) => {
    const res = await axios.post(`${API}/cart/add`, { productId, quantity });
    setCart(res.data.cart); return res.data;
  };
