...
const ProductForm = ({ initialData, onSubmit, onCancel, loading }) => {
  ...
  const validate = () => {
    ...
    if (!form.productName.trim()) e.productName = 'Display name is required';
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:20 }}>
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        ...
        <div onClick={() => fileInputRef.current.click()} style={{ height: imagePreview ? 180 : 120, borderRadius:'var(--radius-md)', border:`2px dashed ${errors.image ? '#ef4444' : 'rgba(14,165,233,0.25)'}`, background:'rgba(14,165,233,0.03)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', transition:'all 0.2s', position:'relative' }}
          onMouseEnter={e => e.currentTarget.style.borderColor='rgba(14,165,233,0.5)'}
          onMouseLeave={e => e.currentTarget.style.borderColor= errors.image ? '#ef4444' : 'rgba(14,165,233,0.25)'}>
          {imagePreview ? (
            <><img src={imagePreview} alt="Preview" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
            <div style={{ position:'absolute', inset:0, background:'rgba(14,165,233,0.4)', display:'flex', alignItems:'center', justifyContent:'center', opacity:0, transition:'opacity 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.opacity=1} onMouseLeave={e => e.currentTarget.style.opacity=0}>
              <span style={{ color:'#fff', fontSize:14, fontWeight:700 }}>Click to change</span>
            </div></>
          ) : (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        <Field label="Product SKU / Code" name="name" placeholder="e.g. SONY-WH1000" required value={form.name} onChange={handleChange} error={errors.name} />
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        <Field label="Shop Name" name="shopName" placeholder="e.g. TechStore Pro" required value={form.shopName} onChange={handleChange} error={errors.shopName} />
        <Field label="Stock Quantity" name="stock" type="number" min="0" placeholder="100" value={form.stock} onChange={handleChange} error={errors.stock} />
      </div>

      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        <label className="field-label">Rating: {form.rating}/5</label>
        <input type="range" name="rating" min="1" max="5" step="1" value={form.rating} onChange={handleChange} style={{ width:'100%', accentColor:'#0ea5e9', height:4 }} />
        <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'var(--text4)' }}>{[1,2,3,4,5].map(n => <span key={n}>{n}★</span>)}</div>
      </div>

      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 18px', background:'rgba(14,165,233,0.04)', borderRadius:'var(--radius-md)', border:'1px solid rgba(14,165,233,0.12)' }}>
        <div>
          <div style={{ fontSize:14, fontWeight:700, color:'var(--text)' }}>Free Delivery</div>
        ...
        <label style={{ position:'relative', display:'inline-block', width:44, height:24, cursor:'pointer' }}>
          ...
          <span style={{ position:'absolute', inset:0, borderRadius:12, transition:'0.3s', background: form.freeDelivery ? 'linear-gradient(135deg,#0ea5e9,#6366f1)' : 'rgba(0,0,0,0.15)' }}>
            <span style={{ position:'absolute', left: form.freeDelivery ? 22 : 2, top:2, width:20, height:20, background:'#fff', borderRadius:'50%', transition:'0.3s', boxShadow:'0 1px 4px rgba(0,0,0,0.2)' }}></span>
          </span>
        </label>
      </div>
